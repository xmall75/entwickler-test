const text = `Dalam kehidupan suatu negara, pendidikan memegang peranan yang amat
...
const arrayText = text.split(' ').join(',').split('.').join(',').split(',') // ambil per kata, dipisahkan melalui spasi lalu pisahkan lagi titiknya

const findSpecificWord = (cari_kata) => {
    return arrayText.filter(kata => kata.toLowerCase() === cari_kata)
}

console.log('Jumlah kata "dan": ', findSpecificWord('dan').length)
console.log('Jumlah kata "untuk": ', findSpecificWord('untuk').length)
console.log('Jumlah kata "tabungan": ', findSpecificWord('tabungan').length)